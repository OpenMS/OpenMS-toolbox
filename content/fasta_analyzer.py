...
def main():
    ...
    **Analyze FASTA sequence files** to compute statistics about sequence lengths and residue composition.
    ...
    - **Detect sequence types** automatically (protein, DNA, or RNA)
    ...
    with st.expander("How FASTA Analysis Works"):
        ...
        **Sequence Type Detection:**
        - Sequences are automatically classified as protein, DNA, or RNA based on their composition
        - Characters unique to proteins (D, E, F, H, I, K, L, M, P, Q, R, S, V, W, Y) indicate protein sequences
        - Sequences with >90% nucleotide characters (A, C, G, T, U) are classified as nucleotide
        - Presence of U without T indicates RNA; otherwise DNA is assumed
    ...
    seq_type = st.selectbox(
        "Sequence type",
        options=["Auto-detect", "Protein", "DNA", "RNA"],
        index=0,
        help="Override automatic sequence type detection if needed",
    )
    ...
    if submit:
        ...
        # Map selection to analysis parameter
        seq_type_map = {
            "Auto-detect": "auto",
            "Protein": "protein",
            "DNA": "dna",
        ...
        selected_type = seq_type_map[seq_type]

        # Perform analysis
        with st.spinner("Analyzing sequences..."):
            try:
                results = analyze_fasta(fasta_input, selected_type)
        ...
        with col2:
            st.metric("Detected Type", results["detected_type"].upper())
        with col3:
